...
def reduced(corr_matrix):
    """[Takes correlation matrix in form of DataFrame and sets lower triangular values and diagonal values to 0.]
...
def get_values(df, exclude):
    ...
    Args:
        ...
        exclude ([String or Boolean]): [Designated parameter which is not of interest and can be dropped. Set to False to keep all.]

    Returns:
        [np.ndarray]: [np.ndarray containing the values and information about what was correlated.]
    """
    list_of_entries = []                              
    ...
    array = np.array(list_of_entries)     #list containing what was correlated and related values
    return array

def sort_values(array_in):
    """[Takes a np.ndarray containing N rows and 3 columns, sorts the rows by absolute value of the entry in the third column.]
...
def get_columns(df, x, y):
    ...
    """
    df_out = pd.DataFrame(index=df.index, columns=[x, y])  #extract the interesting columns ("vectors") into separate dataframes
    df_out[x] = df[x]
    df_out[y] = df[y]    
    return df_out
